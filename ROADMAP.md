...
- **Household sharing — built, pending DB setup ⏳:** code shipped so two logins can share one dataset. **To activate: run `supabase/household-setup.sql` once in the Supabase dashboard (SQL Editor → paste → Run).** It creates `households` + `household_members`, security-definer RPCs (`ensure_household`, `join_household`, `my_household`), adds `household_id` to `app_state`, migrates existing rows, and switches RLS to household-membership. The client is backward-compatible (falls back to user-keyed sync until the SQL is applied, so nothing breaks meanwhile). To link Yona: he signs up normally, then in **Settings → "משק בית משותף"** enters the invite code shown in your Settings (joining replaces his data with the household's).
